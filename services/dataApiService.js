...
app.get("/api/v2/rooms", async (req, res) => {
  try {
    ...
    // PostgreSQL rooms is the current operational snapshot. The Notion → PostgreSQL
    // synchronizer updates these fields in both directions, so do not override them
    // with historical operations_logs events. A completed event may remain in the
    // log even after an administrator manually resets pre-inspection in Notion.
